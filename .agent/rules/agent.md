...
$existingIssues = (Get-Content ...agent\troubleshooting\known-issues.yaml | Select-String -Pattern "id: $category-" | Measure-Object).Count
...
Add-Content -Path ...agent\troubleshooting\known-issues.yaml -Value $newIssue

# Create resolution document from template
Copy-Item ...agent\troubleshooting\_template.md ...agent\troubleshooting\resolutions\$newId.md

# Fill in basic info
$templateContent = Get-Content ...agent\troubleshooting\resolutions\$newId.md -Raw
...
Set-Content -Path ...agent\troubleshooting\resolutions\$newId.md -Value $templateContent
```
...
```powershell
# Update YAML status
$yamlContent = Get-Content ...agent\troubleshooting\known-issues.yaml -Raw
$yamlContent = $yamlContent -replace "id: $newId.*?resolution_status: investigating", "id: $newId`n    resolution_status: solved"
Set-Content -Path ...agent\troubleshooting\known-issues.yaml -Value $yamlContent
...
1. **Always search first** - Check `...agent/troubleshooting/known-issues.yaml` before trying random solutions
...
### 4. PHASE 4: CODE (DSP)
*   **Trigger:** `/impl [Name]`
*   **Skill File:** `...agent/skills/skill_implementation/SKILL.md`
*   **Action:** Implement `PluginProcessor.cpp` and bind parameters to UI.
*   **State:** `code_complete`

### 5. PHASE 5: SHIP (Release)
*   **Trigger:** `/ship [Name]`
*   **Skill File:** `...agent/skills/skill_packaging/SKILL.md`
...
**ERROR HANDLING:**
If any command fails, **CHECK KNOWN ISSUES FIRST** before attempting random solutions. Search `...agent/troubleshooting/known-issues.yaml` for matching error patterns. If found, apply documented solution. If not found and after 3 attempts, trigger auto-capture protocol.
